...
class HCAS_Model(MLPNet):
    def __init__(self, path, is_double=False):
        super(HCAS_Model, self).__init__([3]+[25]*5+[5])
        network = NNet(path)
        with torch.no_grad():
            for i, layer in enumerate(self.layers):
                if is_double:
                    weight, bias = torch.from_numpy(network.weights[i]), torch.from_numpy(
                        network.biases[i])
                else:
                    weight, bias = torch.from_numpy(network.weights[i]).float(), torch.from_numpy(network.biases[i]).float()
...
if __name__ == '__main__':
    net = HCAS_Model('TrainedNetworks/HCAS_rect_v6_pra0_tau00_25HU_3000.nnet')
